...
# Main target
main: $(SRC_FILES) $(CORE_FILES) $(STORAGE_FILES) $(PARSER_FILES) $(EXECUTOR_FILES) $(UTILS_FILES) $(TOKENIZER_FILES) $(PARSER_STATEMENTS_FILES) $(EXECUTOR_STATEMENTS_FILES) $(SEMANTIC_ANALYZER_FILES) $(SEMANTIC_ANALYZER_VALIDATORS_FILES) 
...
test_create_table: $(TEST_CREATE_TABLE) $(TEST_UTILS) $(CORE_FILES) $(STORAGE_FILES) $(PARSER_FILES) $(TOKENIZER_FILES) $(PARSER_STATEMENTS_FILES) $(UTILS_FILES) $(SEMANTIC_ANALYZER_FILES) $(SEMANTIC_ANALYZER_VALIDATORS_FILES)
	$(CC) $(CFLAGS) $^ -o $(TEST_CREATE_TABLE_EXEC)
	valgrind --leak-check=full --show-leak-kinds=all ./$(TEST_CREATE_TABLE_EXEC)

test_drop_table: $(TEST_DROP_TABLE) $(TEST_UTILS) $(CORE_FILES) $(STORAGE_FILES) $(PARSER_FILES) $(TOKENIZER_FILES) $(EXECUTOR_STATEMENTS_FILES) $(PARSER_STATEMENTS_FILES) $(UTILS_FILES) $(SEMANTIC_ANALYZER_FILES) $(SEMANTIC_ANALYZER_VALIDATORS_FILES)
	$(CC) $(CFLAGS) $^ -o $(TEST_DROP_TABLE_EXEC)
	valgrind --leak-check=full --show-leak-kinds=all ./$(TEST_DROP_TABLE_EXEC)

test_insert_row: $(TEST_INSERT_ROW) $(TEST_UTILS) $(CORE_FILES) $(STORAGE_FILES) $(PARSER_FILES) $(TOKENIZER_FILES) $(EXECUTOR_STATEMENTS_FILES) $(PARSER_STATEMENTS_FILES) $(UTILS_FILES) $(SEMANTIC_ANALYZER_FILES) $(SEMANTIC_ANALYZER_VALIDATORS_FILES)
